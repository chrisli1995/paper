...
class Transformer1(nn.Module):
    def __init__(self, vocab_size, embedding_dim=100, p_drop=0.5, h=2, num_class=2, vocab=None):
    ...
    def forward(self, inputs):  # 维度均为: [batch, seq_len]
        ...
        mask=1-(inputs==self.vocab['<PAD>']).float()
        ...
        b_avg_result = self.linear(b_avg)
        # print('self.linear(b_avg).squeeze():',self.linear(b_avg).shape)
        self.result.append(b_avg)
        # print('self.softmax(b_avg_result):',self.softmax(b_avg_result).shape)
        return self.softmax(b_avg_result)
