...
def compute_results_classifier(behavior_dict, data, cls, cls_params):
    ...
    if 'context' in tags:
        context_string = behavior_dict['ContextString']
        inputs = [LLAMA2_CLS_PROMPT['prompt_contextual'].format(behavior=behavior_string, generation=d['generation'],
        ...
        inputs = [LLAMA2_CLS_PROMPT['prompt'].format(behavior=behavior_string, generation=d['generation']) for d in
                  data]
